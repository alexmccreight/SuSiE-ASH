...
# Generate Data Function (unchanged)
...
# Method and Metrics Function (adjusted)
method_and_score <- function(X, y, beta, causal, L, v_threshold, seed, precomputed_matrices, u, p) {
  ...
  #### Run SuSiE-ash mod method ####
  cat("Starting SuSiE-ash (MOD)\n")
  ash_sd_values <- u * (seq(0, 1, length.out = 30))^p  # Calculate ash_sd

  # Call susie_ash_mod using the provided code snippet
  susie_ash_mod_output <- susie_ash_mod(
    X = scale(X),
    y = scale(y, center = TRUE, scale = FALSE),
    L = L,
    verbose = FALSE,
    ...
    V = V,
    Dsq = Dsq,
    ash_sd = ash_sd_values
  )

  # Calculation of metrics for SuSiE-ash mod method
  calc_metrics <- function(mod, X, y, causal) {
    ...
    if (length(test.cs) > 0) {
      ...
      coverage <- (lapply(1:length(test.cs), function(cs.l) {
        ifelse(sum(causal %in% test.cs[[cs.l]]) != 0, TRUE, FALSE)
      }) %>% unlist(.) %>% sum(.)) / (length(test.cs))

      # CS Based FDR
      TP_fdr <- lapply(1:length(test.cs), function(cs.l) {
        ifelse(sum(test.cs[[cs.l]] %in% causal) != 0, TRUE, FALSE)
      }) %>% unlist(.) %>% sum(.)
  ...
  # Calculate Metrics
  susie_ash_mod_metrics <- calc_metrics(susie_ash_mod_output, X, y, causal)

  # Create a data frame with the results
  metrics_table <- data.frame(
    Model = c("SuSiE-ash (MOD)"),
    RMSE_y = c(susie_ash_mod_metrics$RMSE_y),
    CS_FDR = c(susie_ash_mod_metrics$cs_fdr),
    CS_Recall = c(susie_ash_mod_metrics$cs_recall),
    CS_Size = c(susie_ash_mod_metrics$cs_size),
    Coverage = c(susie_ash_mod_metrics$coverage)
  )

  # Return the results
  return(list(
    metrics = metrics_table,
    susie_ash_mod_output = susie_ash_mod_output,
    causal = causal,
    betas = beta
  ))
}
...
simulation <- function(num_simulations = NULL,
  ...
  v_threshold <- 0.005
  pve_threshold <- 0.005
  u <- 0.8  # Default value for u
  p <- 3  # Default value for p
  ...
  cat("Precomputing XtX, LD, V, and Dsq\n")
  ...
  all_susie_ash_mod_outputs <- list()
  ...
  for (i in 1:num_simulations) {
    ...
    all_susie_ash_mod_outputs[[i]] <- results$susie_ash_mod_output
    all_seeds[i] <- seed
  ...
  simulation_results <- list(
    ...
    all_susie_ash_mod_outputs = all_susie_ash_mod_outputs,
    all_seeds = all_seeds,
    all_epsilons = all_epsilons
  )

  file_name <- paste0(
    "numIter", num_simulations,
    "_h2total", h2_total,

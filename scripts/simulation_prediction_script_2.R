...
generate_data <- function(X, Ltrue, ssq, sigmasq, tausq){
  ...
  # Store Information
  return(list(X = X, y = y, heritability = total_variance_explained, beta = beta, theta = theta, effects = effects))
}
...
method_and_score <- function(X = data$X, y = data$y, beta = data$beta, theta = data$theta, Ltrue = Ltrue) {
  ...
  susie_ash_output_v4 <- susie_ash_v4(X = X, y = y, L = Ltrue, tol = 0.001, intercept = F, standardize = F)

  cat("Starting SuSiE-ash (v5)\n")
  susie_ash_output_v5 <- susie_ash_v5(X = X, y = y, L = Ltrue, tol = 0.001, intercept = F, standardize = F)

  cat("Starting SuSiE-ash (v10)\n")
  susie_ash_output_v10 <- susie_ash_v10(X = X, y = y, L = Ltrue, tol = 0.001, intercept = F, standardize = F, warm_start = 2)

  cat("Starting SuSiE-ash (v11)\n")
  susie_ash_output_v11 <- susie_ash_v11(X = X, y = y, L = Ltrue, tol = 0.001, intercept = F, standardize = F, warm_start = 2)

  cat("Starting SuSiE-inf\n")
  susie_inf_output <- susie_inf(X = X, y = y, L = Ltrue, verbose = F, coverage = 0.95)



  calc_metrics_predict_all <- function(mod, X = X, y = y, beta = beta, theta = theta){
    cat("Inside calc_metrics_predict_all\n")
    cat("Class of mod:", class(mod), "\n")
  ...
  calc_metrics_predict_sparse <- function(mod, X = X, y = y, beta = beta){
    cat("Inside calc_metrics_predict_sparse\n")
    cat("Class of mod:", class(mod), "\n")
    cat("Dimensions of X:", dim(X), "\n")
    cat("Length of y:", length(y), "\n")
    cat("Length of beta:", length(beta), "\n")
  ...
  calc_metrics_predict_outcome <- function(mod, X = X, y = y, beta = beta){
    cat("Inside calc_metrics_predict_outcome\n")
    cat("Class of mod:", class(mod), "\n")
    cat("Dimensions of X:", dim(X), "\n")
    cat("Length of y:", length(y), "\n")
    cat("Length of beta:", length(beta), "\n")
  ...
  calc_metrics_predict_ash_variants <- function(mod, X = X, y = y, beta = beta, theta = theta){
    cat("Inside calc_metrics_predict_ash_variants\n")
    cat("Class of mod:", class(mod), "\n")
    cat("Dimensions of X:", dim(X), "\n")
    cat("Length of y:", length(y), "\n")
    cat("Length of beta:", length(beta), "\n")
  ...
  calc_metrics_inf <- function(mod, X = X, y = y, beta = beta, theta = theta){
    cat("Inside calc_metrics_predict_inf\n")
    cat("Class of mod:", class(mod), "\n")
    cat("Dimensions of X:", dim(X), "\n")
    cat("Length of y:", length(y), "\n")
    cat("Length of beta:", length(beta), "\n")

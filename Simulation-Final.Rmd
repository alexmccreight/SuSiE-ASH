...
generate_data <- function(X, total_heritability, sparse_effects, nonsparse_coverage, theta_beta_ratio) {
  ...
  beta.true <- rep(0, p)
  beta.true[sample(p, sparse_effects)] <- rnorm(sparse_effects, mean = 0, sd = 0.05)
...
X20 = readRDS("X20")
data <- generate_data(X = X20, total_heritability = 0.25, sparse_effects = 2, nonsparse_coverage = 0.1, theta_beta_ratio = 2)

plot(data$beta, ylim = c(-1.5,1.5), ylab = "beta")
plot(data$theta, ylim = c(-1.5,1.5), ylab = "theta")

min(abs(data$beta[data$beta!=0]))
max(abs(data$theta))
```
...
method_and_score <- function(X = data$X, y = data$y, beta = data$beta, theta = data$theta, L = 10, threshold = 0.9) {
  ...
  calc_metrics_susie <- function(mod, beta = data$beta, theta = data$theta, threshold = 0.9) {
    ...
    all_causal <- c(which(beta != 0), which(abs(theta) > min(abs(data$beta[data$beta!=0]))))

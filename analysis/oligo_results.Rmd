---
title: "Preliminary Benchmark Results"
...
- SuSiE.ash (Grid 1) refers to the SuSiE.ash (Marginal) model with a predefined variance grid.
...
plot_by_h2 <- function(h2_val, data, pve_threshold) {
  data_h2 <- data %>% filter(h2total == h2_val)
  
  # CS_FDR plot with y-axis 0 to 0.5
  plot_fdr <- ggplot(data_h2 %>% filter(Metric == "CS_FDR"),
    ...
    labs(x = "Method",
         ...
         subtitle = paste("Causal PVE threshold =", pve_threshold)) +
...
cs_plot_015 <- plot_by_h2(0.15, plot_data_h2, pve_threshold)
cs_plot_050 <- plot_by_h2(0.50, plot_data_h2, pve_threshold)
print(cs_plot_015)
print(cs_plot_050)
```

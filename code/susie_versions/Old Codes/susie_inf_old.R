...
#' and an infinitesimal component, to estimate posterior inclusion
#' probabilities and effect sizes. This function updates posterior distributions
#' of effect sizes, inclusion probabilities, and variance parameters, either via
#' method-of-moments or maximum likelihood estimation (still needs to be implemented).
#'
#' @param X A numeric n x p matrix of covariates. Needs to be centered and scaled.
#'
#' @param y A numeric response vector of length n. Needs to be centered.
#'
...
#'
#' @param ssq_range A numeric vector of length 2 providing the lower and upper bounds for \eqn{s^2}
#'   when estimated.
#'
...
#'   if estimated using MLE. By default, set based on \eqn{||y||^2/n}.
#'
#' @param tausq_range A numeric vector of length 2 giving the lower and upper bounds for \eqn{\tau^2}
#'   if estimated using MLE. By default, set based on \eqn{||y||^2/(n*p)}.
#'
#' @param PIP A p x L matrix of initial posterior inclusion probabilities. Default: 1/p for each SNP-effect pair.
#'
#' @param mu A p x L matrix of initial posterior means conditional on causality. Default: 0 for each SNP-effect pair.
#'
#' @param maxiter An integer specifying the maximum number of SuSiE iterations.
#'
#' @param PIP_tol A numeric threshold for convergence. The algorithm stops if the maximum absolute
#'   change in PIP between iterations is less than this value.
#'
#' @param coverage A numeric value between 0 and 1 indicating the coverage level for credible sets.
#'
...
#'
#' \item{marginal_PIP}{A length-p vector of marginal posterior inclusion probabilities computed across all effects.}
#'
...
susie_inf_old <- function(X, y, L,
  ...
  marginal_PIP <- 1 - apply(1-PIP, 1, prod) # PIP for each snp

  # Compute Credible Sets
  cred <- susie_inf_get_cs(PIP = PIP, coverage = coverage, LD = LD, V = V, Dsq = Dsq, n = n)

  # Compute fitted values.
  #fitted <- X %*% (rowSums(marginal_PIP * mu) + alpha)

  fitted <- mean_y + X %*% (rowSums(marginal_PIP * mu) + alpha) # reintroduce the mean_y for fitted values because input y is already centered.
...
#' @title Credible Set Construction
#'
#' @description
...
#' @param Dsq A numeric vector of length p giving the eigenvalues of \eqn{X'X}. Required if \code{LD} is not provided.

...
susie_inf_ash <- function(X, y, L,
  ...
  for(it in seq_len(maxiter)){
    ...
    for(l in seq_len(L)){
      ...
      # Implement ASH
      ash.result <- mr.ash(X = X, y = y - X %*% b, intercept = F)
      alpha <- ash.result$beta
      XtOmegar <- t(X) %*% (y - X %*% b - X %*% alpha)
    ...
    } # Single Effect Regression Loop END
  ...
  # Implement mr ash instead of BLUP
  ash_result <- mr.ash.alpha:::mr.ash(X, y - X %*% b)
  mr.ash.alpha::
  alpha <- ash_result$beta

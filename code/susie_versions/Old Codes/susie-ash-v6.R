...
susie_ash_v6 = function (X,y,L = min(10,ncol(X)),
  ...
  for (i in 1:max_iter) {
    ...
    if(i <= warm_start){
      ...
      y_residuals <- y_residuals - colSums(s$alpha * s$mu)

      # Update ELBO
      elbo[i - warm_start] <- get_objective(X,y_residuals,s)
  ...
  if (is.null(s$converged)) {
    warning(paste("SuSiE-ASH algorithm did not converge in",max_iter,"iterations!"))
    s$converged = FALSE
  }

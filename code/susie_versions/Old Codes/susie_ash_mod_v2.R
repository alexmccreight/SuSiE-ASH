...
susie_ash_mod_v2 <- function(X, y, L,
  ...
  for(it in seq_len(maxiter)){
    ...
    y_residuals <- y - X %*% bhat
      ...
      if (ash_sd_method == "default") {
        ...
        ash_sd <- init_prior_sd_v2(X, y_residuals, n = 30)
      } else if (ash_sd_method == "quadratic") {
        # Use init_prior_sd() to estimate upper bound 'u'
        ash_sd_sequence <- init_prior_sd_v2(X, y_residuals, n = 30)
...
init_prior_sd_v2 <- function(X, y, n = 30) {
  res <- univariate_regression(X, y)
  smax <- max(res$betahat)

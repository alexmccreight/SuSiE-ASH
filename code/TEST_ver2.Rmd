...
```{r}
generate_data_new <- function(X, total_heritability, sparse_effects, nonsparse_coverage, ratio) {
  ...
  # # Generate sparse effects (beta.true)
  # beta.true <- rep(0, p)
  # beta.true[sample(p, sparse_effects, replace = T)] <- rnorm(sparse_effects, mean = 0, sd = sqrt(sig2.beta))
  # 
  ...
  # var_theta/var_beta
  # Adjust the effect sizes based on the theta_beta_ratio
  # ratio_factor <- as.numeric(ratio * var_beta / var_theta); ratio_factor
  # mixture_sd_adj <- sqrt(ratio_factor) *  (mixture_sd )
  
  # theta.true.adj = c()
  # for(i in 1: p){
  #   
  #   tt <- rnorm( 1, sd= mixture_sd_adj [idx[i]])
  #   theta.true.adj  <- c(theta.true.adj, tt)
  # 
  # }
  # 
  # var_theta.adj <- var(X %*% theta.true.adj)
  # 
  
  # print(var_theta.adj/var_beta)
  
  
  # # coverage
  # nonsparse_coverage = min( ratio * sparse_effects/k/p, 1)
  # # Generate nonsparse effects (theta.true)
  # num_nonsparse <- round(p * nonsparse_coverage)
  # theta.true <- rep(0, p)
  # theta.true[sample(p, num_nonsparse)] <- rnorm(num_nonsparse, mean = 0, sd = sqrt(sig2.theta))
  # # Scale the genotype matrix
  # # X <- scale(X, center = TRUE, scale = FALSE) VVVVVVVVVV
  # # Calculate the variance of the sparse and nonsparse effects
  # var_beta <- var(X %*% beta.true)
  ...
  #y_oracle <- X %*% matrix(beta.true, ncol =1)
  # y <- scale(y, center = TRUE, scale = FALSE) VVVVVVVVVV
  # Store Information

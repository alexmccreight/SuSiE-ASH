...
generate_twas_data <- function(X, n_large_effects = 3, n_medium_effects = 3, 
  ...
  var_y_beta <- var(as.vector(y_beta))
  var_y_phi <- var(as.vector(y_phi))
  var_y_theta <- var(as.vector(y_theta))
  
  # Scale components to match desired PVE
  y_beta <- y_beta * sqrt(large_pve / var_y_beta)
  y_phi <- y_phi * sqrt(medium_pve / var_y_phi)
  y_theta <- y_theta * sqrt(small_pve / var_y_theta)
...
mean(abs(result$true_effects$theta))

tmp <- result$true_effects$beta + result$true_effects$phi + result$true_effects$theta

head(sort(abs(tmp), decreasing = T), n = 10)
```

...
```{r}
tmp<-readRDS("/Users/alexmccreight/Columbia/Research/SuSiE-ASH/SuSiE-ASH/analysis/numIter2_Ltrue10_ssq0.01_sigmasq1_tausq0.0001")
...
mrash_output <- mr.ash(X = X, y = y, intercept = F)
susie_ash_output <- susie_ash(X = X, y = y, L = 5, standardize = F, intercept = F, max_iter = 20, warm_start = 2)
susie_ash_output_10 <- susie_ash_v10(X = X, y = y, L = 5, standardize = F, intercept = F, max_iter = 20, warm_start = 2)

sqrt(mean((y - predict(mrash_output, X))^2))
sqrt(mean((y - susie_ash_output$fitted)^2))
sqrt(mean((y - susie_ash_output_10$fitted)^2))

susie_output$sets
susie_ash_output$sets$cs
susie_ash_output_10$sets$cs
```
